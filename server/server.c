...
void *handle_client(void *arg){
	...
	// Record protocol exchange completed
	char message[255] = "Client protocol exchange completed.\n\0";
	write_to_log(message);
...
int protocol_exchange(int connect_fd){
	...
	if((nwrite = write(connect_fd, server_protocol, strlen(server_protocol))) == -1){
		// Record client no longer listening
	    char message[255] = "Unable to communicate protocol to client.\n\0";
	    write_to_log(message);
	...
	if((nread = read(connect_fd,from_client,512)) == -1){
		char message[255] = "Client did not respond to protocol.\n\0";
		write_to_log(message);
	...
	if(strcmp(from_client, SECRET) != 0){
		// Record client unable to access server
	    char message[255] = "Client submitted incorrect secret.\n\0";
	    write_to_log(message);
	...
	// Record client accessed server
	char message[255] = "Client submitted correct secret.\n\0";
	write_to_log(message);
...
void write_to_log(char *write_sentence){
	...
	char date[22]; // space enough for YYYY-MM-DD-server.log
	struct tm tm;
	time_t current_time;
